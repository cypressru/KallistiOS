...
semaphore_t sem;
static volatile uint32_t filler_checksum;
...
void *thd_0(void *v) {
    ...
    /* Preserve the original example's non-blocking filler workload without
       depending on Dreamcast video RAM. */
    for(i = 0; i < 320U * 480U; ++i)
        checksum = (checksum << 5) - checksum + i;
    filler_checksum = checksum;
...
int main(int argc, char **argv) {
    ...
    thd_join(t0, NULL);
    printf("Filler checksum: %08lx\n", (unsigned long)filler_checksum);
